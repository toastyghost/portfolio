<?
	...
	$services = $mdb->fetch('select * from mimik_Services');
	foreach($services as $service) $temp_services[$service['name']] = $service['id'];
	$services = $temp_services;
	unset($temp_services);
	
	if(strpos($_SERVER['REQUEST_URI'],'services')!==false){
		if($The_Landing_Page) $temp_page = $The_Landing_Page;
		elseif($The_Interior_Page) $temp_page = $The_Interior_Page;
		$service_filter = $services[$temp_page['title']];
		if($service_filter || $temp_page['title'] == 'Services'){
			$num_chars = strlen(strip_tags($temp_page['content']));
			$number_of_height_increasing_elements = substr_count($temp_page['content'],'<p>')+substr_count($temp_page['content'],'<ul>')+substr_count($temp_page['content'],'<li>');
			unset($temp_page);
			...
			while($i<$num_pics){
				...
				if($service_filter) $project_filter_sql = ' service = "'.$service_filter.'" and';
